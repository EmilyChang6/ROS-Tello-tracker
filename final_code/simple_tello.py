...
class Tello_drone():
    ...
    def _sensor(self):
        ...
        _ap_sub = rospy.Subscriber("/target_ap", Float64MultiArray, self._ap_cb, queue_size = 10)
    ...
    # _tp_sub 的 接收訊息function, 用於接收來自/target_point 的訊息, 其表示目前抓到的HSV mask 中心點 與mask面積是否超過門檻值
    def _ap_cb(self, data):
        
        # 使用 self.state裡的變數來儲存
        self.state.target_x = data.data[0]  # 藉由訂閱topic: /target_ap 所獲得的框中心點 x
        self.state.target_y = data.data[1]  # 藉由訂閱topic: /target_ap 所獲得的框中心點 y
        self.state.canTag = data.data[2]   # 藉由訂閱topic: /target_ap 所獲得是否mask面積超過門檻值
